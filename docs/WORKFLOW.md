...
- 所有层级的 `results/`、`analysis_results/`、`validation_results/` 允许提交图表和摘要。
  新的 `data/`、`spectra/`、逐次 FFT、原始 TDMS、NPZ/MAT/FIG 及完整分辨率频谱 CSV 不入库；
  原有参考数据继续用于回归测试。后续确需归档的最终二进制产物应单独审查并使用 Git LFS。
